...
async def run():
    async with grpc.aio.insecure_channel('localhost:50051') as channel:
        stub = schedule_pb2_grpc.SchedulesServiceStub(channel)
        ...
        if rpc_call == "1":
            ...
            request = schedule_pb2.CreateScheduleRequest(  # type: ignore
            ...
            print(reply)
        if rpc_call == "2":
            request = schedule_pb2.GetAllSchedulesRequest(user_id=1)  # type: ignore
            reply = await stub.GetAllSchedules(request)
            print("Get All Schedules Response Received:")
            print(reply)
        if rpc_call == "3":
            request = schedule_pb2.GetUserScheduleRequest(user_id=1, schedule_id=2)  # type:ignore
            reply = await stub.GetUserSchedule(request)
            print("Get User Schedule Response Received.")
            print(reply)

        if rpc_call == "4":
            request = schedule_pb2.GetUserNextTakingsRequest(user_id=1)  # type: ignore
